 ...
 * @param {Array<{id:string,ordered_at?:string,status?:string,order_notes?:unknown,confirmed_map?:unknown}>} orders
...
export function denormalizeOrders(orders, orderItems) {
  ...
  for (const o of orders || []) {
    const rawMap = o && typeof o.confirmed_map === 'object' && o.confirmed_map && !Array.isArray(o.confirmed_map)
      ? o.confirmed_map
      : {};
    const normalizedMap = {};
    for (const [itemId, val] of Object.entries(rawMap)) {
      if (String(itemId || '').trim() !== '' && !!val) normalizedMap[String(itemId)] = true;
    }
    ordMap[o.id] = {
      ...o,
      order_notes: o?.order_notes != null ? String(o.order_notes) : '',
      confirmed_map: normalizedMap,
      items: {},
    };
  }
